  ...
  <div>
    <div class="banner bg-4">
      <div class="banner-cont down">
        <div class="banner-title">医院人员情况表</div>
        <div class="banner-subhead mt40">健康管理中心现有主检医师2名、内科12名、外科10名、妇科5名、眼科6名、耳鼻喉科4名、口腔科4名、心电图2名、B超13名，放射9名、抽血检验20名、测血压2名，身高体重2名，导检护士20。其中高级职称23人，中级职称28人，体检各岗位人员配置数量、职称和经验完全满足招标要求。</div>
      </div>
    ...
    <div class="content-wrap">
      ...
      <div v-if="currentTab == 'tab1'" class="mt20">
        <table class="mt-table">
          ...
          <tr v-for="(tr, idx) in src.tab1" :key="idx">
            <td>{{tr.id}}</td>
            <td>{{tr.name}}</td>
            <td>{{tr.gender}}</td>
            <td>{{tr.age}}</td>
      ...
      <div v-if="currentTab == 'tab2'" class="mt20">
        <div class="img-panel">
          <div class="img-w" v-for="(im, i) in src.tab2" :key="i">
            <img class="img bdr shadow" :src="im" />
          </div>
        </div>
      </div>

      <div v-if="currentTab == 'tab3'" class="mt20">
        <table class="mt-table">
          <tr>
            <th>名称</th>
            <th>规格</th>
          ...
          <tr v-for="(tr, idx) in src.tab3" :key="idx">
            <td>{{tr.name}}</td>
            <td>{{tr.standard}}</td>
            <td>{{tr.num}}</td>
            <td>{{tr.status}}</td>
